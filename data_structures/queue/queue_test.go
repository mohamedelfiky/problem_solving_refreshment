...
func TestPush(t *testing.T) {
	...
	if assert.NotNil(t, q.head) {
		...
		assert.Equal(t, q.rear.value, 5, "they should be equal")
	...
	if assert.NotNil(t, q.head.next) {
		...
		assert.Equal(t, q.rear.value, 6, "they should be equal")
	...
	if assert.NotNil(t, q.head.next.next) {
		...
		assert.Equal(t, q.rear.value, 7, "they should be equal")
